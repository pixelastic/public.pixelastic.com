...
(function($){
	...
	$.swfupload = {
		...
		// Getting an identity card of debug informations
		displayDebugInfo: function() {
			// Base informations
			var ret = { 'Version': SWFUpload.version, 'Movie Name': this.movieName }
			// Getting the settings
			for(var name in this.settings) ret[name] = this.settings[name];
			// Displaying them in debug
			this.debugMessage(ret);
		},
		// Displaying a debug message
		debugMessage: function(message) {
			if (!this.settings.debug) return false;
			return $.log(message, $('#swfUploadPanel'));
		}
	};

	// Dispatch SWF debug methods to the jQuery plugin
	SWFUpload.prototype.displayDebugInfo = $.swfupload.displayDebugInfo;
	SWFUpload.prototype.debugMessage = $.swfupload.debugMessage;
