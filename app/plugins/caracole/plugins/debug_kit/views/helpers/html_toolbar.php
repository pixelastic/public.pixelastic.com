 ...
 * @access public
 */
	var $helpers = array('Html', 'Form', 'Caracole.Fastcode');
/**
 * settings property
 *
 * @var array
 ...
 **/
	function panelStart($title, $anchor) {
		// Getting the panel name to fetch the correct icon
		if (strpos($anchor, '.')>0) {
			list(,$panelId) = explode('.', $anchor);
		} else {
			$panelId = $anchor;
		}
		$link = $this->Fastcode->link($title, '#' . $anchor, array('icon' => 'debug_'.$panelId));
		return $link;
	}
/**
 * Create a table.
 ...
 */
	function table($rows, $headers = array()) {
		$out = '<table class="debug-table" cellspacing="0">';
		if (!empty($headers)) {
