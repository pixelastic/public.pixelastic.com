 ...
 *	Even when not installed, Caracole will try to fix non-writable directory issues
 **/
/*
 Configure::write('Caracole.installed', file_exists(APP.'config'.DS.'installed'));
// Trying to change TMP directory rights
if (!is_writable(TMP) && !Configure::read('Caracole.installed')) {
	if (!class_exists('Folder')) {
		require LIBS . 'folder.php';
	}
	$Folder =& new Folder(TMP);
	if (!$Folder->chmod(TMP)) {
		die(__d('caracole', 'Your app/tmp directory is not writable. The application will not run until it is changed.', true));
	}
