 ...
 */
	function execute() {
		...
		// Caracole add. We pass to __searchFiles if we need to search on plugins
 ...
 */
	function help() {
		...
		// Caracole add
		$this->out(__('   -plugins: [yes|no]: Extracts from inner plugins', true));
		// End Caracole add
		$this->out();
	...
	function __searchFilesExcludePlugins() {
		foreach ($this->__paths as $path) {
			...
			foreach($filelist[1] as &$file) {
				// We discard plugins
				$this->out(str_replace($path, '', $file));
